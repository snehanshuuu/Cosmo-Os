...
export const CyberCatCompanion: React.FC = () => {
  ...
  return (
    <div className="fixed bottom-3 left-3 z-30 flex flex-col items-start select-none group">
      ...
      <AnimatePresence>
        {isExpanded && (
          <motion.div
            ...
            className="mb-2.5 w-76 h-76 rounded-xl bg-black/90 border border-cosmos-lime/60 p-2.5 flex flex-col justify-between shadow-[0_0_25px_rgba(124,255,0,0.3)] backdrop-blur-xl relative pointer-events-auto"
          >
            {/* Component Header Rebranded to C.A.R. */}
            <div className="flex justify-between items-center border-b border-white/10 pb-1.5">
              <div className="flex items-center gap-1.5 font-bold text-[11px] text-white uppercase tracking-wider">
                <Icons.Sparkles className="w-3.5 h-3.5 text-cosmos-lime animate-pulse" />
            ...
            {/* Chat Message Stream */}
            <div className="flex-1 overflow-y-auto my-1.5 flex flex-col gap-1.5 p-1 font-mono text-[11px] scrollbar-thin">
              {chatLog.map((msg) => (
                <div
                  key={msg.id}
                ...
                >
                  <div
                    className={`max-w-[88%] p-2 rounded-lg text-[10px] leading-relaxed ${
                      msg.sender === 'user'
              ...
              {isThinking && (
                <div className="flex justify-start">
                  <div className="bg-white/10 border border-cosmos-lime/40 text-cosmos-lime-bright px-2.5 py-1 rounded-lg text-[10px] font-mono flex items-center gap-1.5 animate-pulse">
                    <span>C.A.R. is thinking...</span>
                    <span className="w-1 h-1 rounded-full bg-cosmos-lime animate-bounce" />
                    <span className="w-1 h-1 rounded-full bg-cosmos-lime animate-bounce [animation-delay:0.2s]" />
                    <span className="w-1 h-1 rounded-full bg-cosmos-lime animate-bounce [animation-delay:0.4s]" />
                  </div>
                </div>
              )}
              <div ref={chatEndRef} />
            ...
            {/* Prompt Input Form */}
            <form onSubmit={handleSend} className="flex gap-1 pt-1.5 border-t border-white/10">
              <div className="flex-1 flex items-center bg-black/70 border border-white/15 rounded-md px-2 py-0.5 text-[11px] font-mono text-white focus-within:border-cosmos-lime">
                <span className="text-cosmos-lime-bright font-bold text-[9px] mr-1 whitespace-nowrap">
                  car@cosmos:~$
                </span>
                <input
                  ref={inputRef}
                  ...
                  disabled={isThinking}
                  className="w-full bg-transparent text-[10px] font-mono text-white focus:outline-none placeholder:text-white/30"
                />
              </div>
              <button
                type="submit"
                disabled={isThinking || !inputVal.trim()}
                className="px-2.5 py-0.5 bg-cosmos-lime text-black font-bold text-[10px] rounded-md hover:bg-cosmos-lime-bright disabled:opacity-40 transition-all flex items-center justify-center shadow-lime-glow"
              >
                Send
              </button>
            </form>
      ...
      <AnimatePresence>
        {!isExpanded && showSpeech && (
          <motion.div
            ...
            className="mb-1.5 max-w-[240px] p-2 rounded-xl bg-black/85 border border-cosmos-lime/50 text-cosmos-lime-bright text-[11px] font-mono shadow-[0_0_12px_rgba(124,255,0,0.2)] backdrop-blur-md relative cursor-pointer pointer-events-auto"
          >
            <div className="flex items-center justify-between gap-1 font-bold mb-0.5 text-white border-b border-white/10 pb-0.5">
              <div className="flex items-center gap-1">
                <Icons.Sparkles className="w-3 h-3 text-cosmos-lime" />
                <span className="font-mono text-cosmos-lime-bright text-[10px]">C.A.R. :: CYBER AI</span>
              </div>
              <button
                onClick={(ev) => {
                  ev.stopPropagation();
            ...
            <p className="text-[10px] text-white/90 leading-tight">{currentTip}</p>
            <span className="text-[8px] font-mono text-cosmos-lime/70 block mt-0.5">
              Click to chat with C.A.R. 💬
            </span>

            {/* Speech Bubble Pointer Arrow */}
            <div className="absolute -bottom-1 left-4 w-2.5 h-2.5 bg-black/85 border-b border-r border-cosmos-lime/50 rotate-45" />
          </motion.div>
        )}
      </AnimatePresence>

      {/* C.A.R. Compact Cyber Cat Avatar Icon (Fixed at Bottom-Left Corner) */}
      <motion.div
        onClick={handleCatClick}
        animate={isBouncing ? { y: [0, -8, 0] } : { y: [0, -3, 0] }}
        transition={
          isBouncing
            ? { duration: 0.4, ease: 'easeOut' }
            : { duration: 3, repeat: Infinity, ease: 'easeInOut' }
        }
        className={`w-10 h-10 rounded-xl bg-black/85 border-2 p-1 flex items-center justify-center relative transition-all cursor-pointer shadow-md pointer-events-auto ${
          avatarState === 'thinking'
            ? 'border-amber-400 shadow-[0_0_20px_rgba(255,193,7,0.6)]'
            : avatarState === 'chatting'
            ? 'border-cosmos-lime shadow-[0_0_20px_rgba(124,255,0,0.6)]'
            : avatarState === 'happy'
            ? 'border-cyan-400 shadow-[0_0_20px_rgba(0,240,255,0.6)]'
            : 'border-cosmos-lime/70 shadow-[0_0_15px_rgba(124,255,0,0.3)] group-hover:border-cosmos-lime group-hover:shadow-[0_0_20px_rgba(124,255,0,0.5)]'
        }`}
      ...
      >
        ...
        <span
          className={`absolute top-0.5 right-0.5 w-2 h-2 rounded-full animate-pulse ${
            avatarState === 'thinking'
              ? 'bg-amber-400 shadow-[0_0_6px_#FFC107]'
              : avatarState === 'happy'
